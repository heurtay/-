 ...
 * Основные классы (как Vehicle, Shape, Animal, User, Validator, Counter) определяют общие свойства и поведение.
 * Производные классы (или подклассы, например, Car, Circle, Fish, PremiumUser, NumberValidator, LimitedCounter) наследуют это от основных классов и могут добавлять новые или изменять существующие (переопределять) методы.
 * Функция issubclass() используется для проверки, является ли один класс подклассом другого.
